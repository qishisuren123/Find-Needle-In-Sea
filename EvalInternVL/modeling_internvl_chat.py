...
class InternVLChatModel(PreTrainedModel):
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config, history=None, return_history=False,
        ...
        print("this is the function chat!")
        img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)
        self.img_context_token_id = img_context_token_id
        if tokenizer.convert_tokens_to_ids('<|im_end|>') != 0:
            eos_token_id = tokenizer.convert_tokens_to_ids('<|im_end|>')  # 92542, InternLM2
    ...
    def generate(
        ...
        if pixel_values is not None:
            ...
            input_embeds[selected] = vit_embeds.reshape(-1, C).to(device)
    ...
    def chatimagetext(self, tokenizer, pixel_values, question, generation_config, pixel_shapes, history=None, return_history=False,
             IMG_START_TOKEN='<img>', IMG_END_TOKEN='</img>', IMG_CONTEXT_TOKEN='<IMG_CONTEXT>'):
        # print("this is the new function chat!")
        img_context_token_id = tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)
        self.img_context_token_id = img_context_token_id
        if tokenizer.convert_tokens_to_ids('<|im_end|>') != 0:
            eos_token_id = tokenizer.convert_tokens_to_ids('<|im_end|>')  # 92542, InternLM2
        ...
        if history is None:
            ...
            # question = image_tokens + '\n' + question
            image_tokens = []
            for i in range(len(pixel_shapes)):
                image_tokens.append(IMG_START_TOKEN + IMG_CONTEXT_TOKEN * self.num_image_token * pixel_shapes[i] + IMG_END_TOKEN + '\n')
            image_place = []
            start_len=0
            s = "<image>"
            while True:
                num=question.find(s,start_len)
                if num==-1:  # 找不到则返回-1
                    break
                image_place.append(num)
                start_len=num+1
            for i in range(len(image_place)):
                replace_place = image_place[len(image_place) - 1 - i]
                if replace_place + 7 >= len(question):
                    questionnew = question[: replace_place] + image_tokens[len(image_place) - 1 - i]
                else:
                    # print(question[: replace_place])
                    # print(image_tokens[len(image_place) - 1 - i])
                    # print(question[replace_place + 7:])
                    questionnew = question[: replace_place] + image_tokens[len(image_place) - 1 - i] + question[replace_place + 7:]
