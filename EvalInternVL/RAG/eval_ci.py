...
device_map = {
    ...
    'language_model.output.weight': 7
}
for i in range(48):
    device_map[f'language_model.model.layers.{i}'] = i // 7 + 1
...
ans_file = open("/mnt/petrelfs/renyiming/LTT/InternVL/Eval/RAG-" + questype + ".jsonl", "w")
...
# for k in range(1):
    # i = 3
    # if len(sample[i]["images_list"]) == 0:
    sample[i]['context'], sample[i]['images_list'], sample[i]['question'], sample[i]['answer'] = get_input(sample[i])
    question = sample[i]['question']
    # print(question)
    images_list = []
    for j in range(len(sample[i]['images_list']) - 1):
        images_list.append('/mnt/petrelfs/renyiming/LTT/NeedleInSea/data/' + sample[i]['images_list'][j])
    images_list.append("/mnt/petrelfs/renyiming/LTT/NeedleInSea/Picture/" + sample[i]["images_list"][-1])
    sample[i]['context'], sample[i]['images_list'] = rag(sample[i]['context'], images_list, question, 3000)
    # print(sample[i])
    pixel_shapes = []
    pixel_value = load_image(sample[i]["images_list"][0], max_num=6).to(torch.bfloat16).cuda()
    pixel_values = pixel_value
    pixel_shapes.append(pixel_value.shape[0])
    for j in range(1, len(sample[i]["images_list"]) - 1):
        pixel_value = load_image(sample[i]["images_list"][j], max_num=6).to(torch.bfloat16).cuda()
        pixel_values = torch.cat((pixel_values, pixel_value), dim=0)
        pixel_shapes.append(pixel_value.shape[0])
    pixel_value = load_image(sample[i]["images_list"][-1], max_num=6).to(torch.bfloat16).cuda()
    pixel_values = torch.cat((pixel_values, pixel_value), dim=0)
    pixel_shapes.append(pixel_value.shape[0])
